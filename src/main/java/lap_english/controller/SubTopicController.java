...
public class SubTopicController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "500", description = "Lỗi server nội bộ.")})
    @PutMapping("/{id}")
    public ResponseData<SubTopicDto> updateSubTopic(@PathVariable Long id,
                                          @RequestPart("data") @Validated(Update.class) SubTopicDto subTopicDto,
                                          @Parameter(
                                                 content = @Content(
                                                          mediaType = "multipart/form-data",
                                                          schema = @Schema(type = "string", format = "binary")
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "500", description = "Lỗi server nội bộ.")})
    @GetMapping
    public ResponseData<PageResponse<List<SubTopicDto>>> getPage(
            ...
            Pageable pageable,
            @Parameter(
                    description = "xắp xếp theo trường nào, mặc định là id, tăng dần, giảm dần bằng asc, desc",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(type = "array", format = "string",defaultValue = "name~animal")
                    ))
    ...
    public ResponseData<PageResponse<List<SubTopicDto>>> getByMainTopic(@PathVariable("mainTopicId") Long mainTopicId,
